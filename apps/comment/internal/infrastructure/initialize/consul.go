...
import (
	...
	"github.com/bhtoan2204/comment/utils"
...
func InitConsul() func() {
	...
	registration := &api.AgentServiceRegistration{
		...
		Port:    servicePort,
		Tags:    []string{"api", "comment"},
		Check: &api.AgentServiceCheck{
			HTTP:                           fmt.Sprintf("http://%s:%d/api/v1/comment-service/health", serviceAddress, servicePort),
			Method:                         "GET",
			Interval:                       "10s",
			Timeout:                        "5s",
			Notes:                          "Basic health check in comment service " + fmt.Sprintf("http://%s:%d/api/v1/comment-service/health", serviceAddress, servicePort),
