...
func InitVideoGRPCClient() {
	resolver.Register(&ConsulResolver{
		...
		serviceName:  "video-grpc",
	})

	conn, err := grpc.NewClient(
		"consul:///video-grpc", // scheme "consul"
	...
	if err != nil {
		global.Logger.Error("Failed to connect to video grpc server", zap.Error(err))
		panic(err)
	}

	global.VideoGRPCClient = video.NewVideoServiceClient(conn)
	global.Logger.Info("Initialize video grpc client successfully")
}
